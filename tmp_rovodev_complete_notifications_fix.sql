-- ПОЛНОЕ ИСПРАВЛЕНИЕ СИСТЕМЫ АВТОМАТИЧЕСКОЙ ОЧИСТКИ УВЕДОМЛЕНИЙ
-- Проблема: прочитанные уведомления не удаляются автоматически через сутки

-- 1. Добавляем поле last_viewed_at если его нет
ALTER TABLE user_notifications 
ADD COLUMN IF NOT EXISTS last_viewed_at TIMESTAMPTZ;

-- 2. Функция для очистки старых прочитанных уведомлений
...
-- 4. Главная функция для вызова из приложения (ИСПРАВЛЯЕТ ОШИБКУ 404)
...
-- 5. Функция для обновления одного уведомления с last_viewed_at
...
-- 6. Функция для массовой отметки уведомлений как прочитанных с last_viewed_at
...
-- 7. Триггер для автоматического обновления last_viewed_at
...
-- 8. Предоставляем права на выполнение функций
...
-- 9. Обновляем существующие прочитанные уведомления без last_viewed_at
UPDATE user_notifications 
SET last_viewed_at = created_at 
WHERE is_read = TRUE AND last_viewed_at IS NULL;

-- 10. Тестируем функцию
SELECT maintenance_cleanup_notifications();

-- 11. Показываем статистику
SELECT 
    ...
    COUNT(*) FILTER (WHERE last_viewed_at IS NOT NULL) as notifications_with_viewed_at,
    COUNT(*) FILTER (WHERE is_read = true AND last_viewed_at < NOW() - INTERVAL '1 day') as ready_for_cleanup
