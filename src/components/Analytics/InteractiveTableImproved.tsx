...
interface ExamGroup {
  ...
  exams: EmployeeExamWithDetails[];
  count: number;
  earliestExam?: EmployeeExamWithDetails;
}
...
const InteractiveTableImproved: React.FC<InteractiveTableProps> = ({ sectionId }) => {
  const { user } = useAuth();
  ...
  const [viewMode, setViewMode] = useState<'compact' | 'expanded'>('compact'); // Режим отображения таблицы
  ...
  // Prepare exam groups with proper sorting
  const examGroups = useMemo(() => {
    const groupMap = new Map<string, ExamGroup>();
    
    employees.forEach(employee => {
      employee.exams.forEach(exam => {
        // Extract prefix (first 3 characters before underscore)
        const prefixMatch = exam.exam_name.match(/^([A-ZА-Я_]{3})/i);
        const groupKey = prefixMatch ? prefixMatch[1].toUpperCase() : exam.exam_name.substring(0, 3).toUpperCase();
        ...
        // Add exam to group only if it's not already there
    ...
    // Process groups: sort exams by date and find earliest exam
    return Array.from(groupMap.values())
      .map(group => {
        // Sort exams in group by next_exam_date (earliest first)
        const sortedExams = [...group.exams].sort((a, b) => {
        ...
        // Find earliest exam (first in sorted array)
        ...
        return {
          ...
          earliestExam: earliestExam
        };
      })
      .sort((a, b) => {
        // Sort groups by minimum periodicity
  ...
  const renderEditableCell = (
    ...
    const changeKey = `${employee.id}-${type}-${examId || ''}`;
    ...
    return (
      <div 
        ...
        onClick={() => {
          // Special handling for name cell - toggle view mode
          if (type === 'name') {
            toggleViewMode();
          } else {
            handleCellEdit(employee.id, type, examId);
          }
        }}
        title={isPending ? 'Изменение ожидает подтверждения' : canEdit ? 
          (type === 'name' ? 'Нажмите для переключения режима отображения' : 'Нажмите для редактирования') : ''}
      >
        {value}
          ...
          <thead className="table-head-fixed">
            <tr>
              ...
              {examGroups.map(group => {
                if (viewMode === 'expanded' || expandedGroups.has(group.key)) {
                  return group.exams.map(exam => (
                    <th key={exam.exam_name} className="col-exam exam-header">
                      {exam.exam_name}
                    </th>
                  ));
                } else {
                  return (
                    <th 
                      key={group.key} 
                      className={`col-group group-header ${expandedGroups.has(group.key) ? 'expanded' : ''}`}
              ...
              <tr key={employee.id} className="table-row">
                ...
                {examGroups.map(group => {
                  if (viewMode === 'expanded' || expandedGroups.has(group.key)) {
                    ...
                    // Group cell - show earliest exam
                    const employeeExamsInGroup = employee.exams.filter(e =>
                      e.exam_name.substring(0, 3).toUpperCase() === group.key
                    );
                    
                    // Sort employee exams by date and find earliest
                    const sortedEmployeeExams = [...employeeExamsInGroup].sort((a, b) => {
                      const dateA = a.next_exam_date ? new Date(a.next_exam_date).getTime() : 0;
                      const dateB = b.next_exam_date ? new Date(b.next_exam_date).getTime() : 0;
                      return dateA - dateB;
                    });
                    
                    const earliestExam = sortedEmployeeExams[0];
                    
                    return (
                      <td
                        key={`${employee.id}-${group.key}`}
                        className={`col-group group-cell ${earliestExam ? getStatusClass(earliestExam.status) : ''}`}
                        onClick={() => toggleGroup(group.key)}
                        title="Нажмите для развертывания группы"
                      >
                        {earliestExam ? formatDate(earliestExam.next_exam_date) : '-'}
...
export default InteractiveTableImproved;